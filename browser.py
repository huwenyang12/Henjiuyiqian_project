...
from utils import try_click, safe_click
    ...
    def login(self):
        try:
            ...
            if ele_logo:
                ...
                return
            print("正在进行登录...")
            cc.find_element(locator.login.login_username).set_text(cfg["login"]["username"])
            time.sleep(1)
            cc.find_element(locator.login.login_password).set_text(cfg["login"]["password"])
            time.sleep(1)
            cc.find_element(locator.login.button_登录).click()
            time.sleep(2)
